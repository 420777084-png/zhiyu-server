...
async function loadManagedContent() {
  try {
    ...
    if (videos.length) {
      ...
      videoGrid.innerHTML = videos.map((item, index) => `<button class="video-card" data-title="${item.title}"><div class="video-art ${['water-video','brain-video','mars-video'][index % 3]}"><span class="video-play">▶</span><span class="duration">科普视频</span></div><span class="tag ${['cyan','purple','orange'][index % 3]}">${item.category}</span><h3>${item.title}</h3><p>${item.summary || '观看医学健康科普视频。'}</p></button>`).join('');
      // 重新绑定视频点击事件
      document.querySelectorAll('.video-card').forEach(card => card.addEventListener('click', () => openVideo(card.dataset.title)));
    }
  } catch (error) {
...
document.querySelector('.play-intro').addEventListener('click', () => openVideo('欢迎来到知愈医学'));
