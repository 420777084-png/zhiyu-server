...
let activeColumn = params.get('name') || '疾病';
const descriptions = { 疾病: '认识疾病信号，了解科学预防和规范诊疗。', 营养: '用可靠的营养知识，建立健康饮食习惯。', 急救: '掌握关键急救常识，紧急时刻正确行动。', 心理: '关注情绪和心理状态，学习科学应对方法。', 用药: '了解常见药物知识，守护安全用药。', 儿童: '陪伴孩子健康成长，正确应对常见问题。' };
...
async function loadData() {
  try {
    ...
    if (!categories.includes(activeColumn)) activeColumn = categories[0] || '疾病';
    render();
  } catch (error) {
...
function render() {
  const list = articles.filter(item => item.category === activeColumn);
  document.title = `${activeColumn}专栏 · 知愈医学`;
  document.querySelector('#column-title').textContent = `${activeColumn}专栏`;
  document.querySelector('#column-description').textContent = descriptions[activeColumn] || `系统了解${activeColumn}相关医疗健康知识。`;
  document.querySelector('#article-total').textContent = list.length;
  const select = document.querySelector('#column-select');
  select.innerHTML = categories.map(name => `<option value="${name}">${name}专栏</option>`).join('');
...
document.querySelector('#column-select').addEventListener('change', event => {
  activeColumn = event.target.value;
  history.replaceState(null, '', `?name=${encodeURIComponent(activeColumn)}`);
  render();
});
